                ...
                <div className="form">
                    <form onSubmit={handleSubmit(onSubmit)} className={font.className}>
                        {inputs.length && inputs.map((input, idx) =>
                            input.type === 'submit' ? (
                                <Button data-wow-dduration="0.6s" key={idx} as="button" type={input.type} label={"Submit form"} classes={`wow fadeIn ${Object.keys(errors).length === 0 ? 'hover:cursor-pointer hover:bg-white hover:text-visited' : 'opacity-25 pointer-events-none'} w-full mt-[25px] md:mt-[35px]`}>{input.value}</Button>
                            ) : (

                                <div data-wow-dduration={`0.${2 + idx}s`} className={`wow fadeIn w-full relative ${idx !== 2 && 'mb-[25px]'} `} key={idx}>
